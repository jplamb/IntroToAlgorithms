...
# def make_chain(G, nodes):
#     for x in range(len(nodes)-2):
#         G = make_link(G, nodes[x], nodes[x+1])
#     return G
# 
# def make_star(G, nodes):
#     main = nodes[0]
#     for x in range(1,len(nodes)):
#         G = make_link(G, main, nodes[x])
#     return G
def create_combo_lock(nodes):
    ...
    for x in range(len(nodes)-1):
        G = make_link(G, nodes[x], nodes[x+1])
        G = make_link(G, main, nodes[x+1])
        print G
   
    #star = make_star(G, nodes)
    #chain = make_chain(G,nodes)
    print is_chain(G, nodes)
    # your code here
    return G
